...
async function start(){
    ...
    await page.waitForSelector('div.prem_list > div > div.premier_item > span.await_rating > i > u', { timeout: 1000 });    //Ожидает прогрузки указанного селектора

    const name = await page.$$eval('div.prem_list > div > div.premier_item > div.text > div > span.name_big > a', postLinks => postLinks.map((el) =>el.innerText)); //Находит все названия
    const sumFilms = name.length;   //Количество фильмов
    const ne = await page.$$eval('div.prem_list > div > div.premier_item > div.text > div > span:nth-child(2)', postLinks => postLinks.map((el) =>el.innerText));   //Находит все названия на английском
    const name_eng=[];
    const film_link = await page.$$eval('div.text > div > span.name_big > a', postLinks => postLinks.map((el) =>el.href));  //Находит ссылка на страницы фильмов
    const rating = await page.$$eval('div.prem_list > div > div.premier_item > span', postLinks => postLinks.map((el) =>el.innerText)); //Находит все рейтинги (фильма и ожидания)
    const film_rating=[];
    const wait_rating=[];
    const votes = await page.$$eval('div.prem_list > div > div.premier_item > span > i > u > b', postLinks => postLinks.map((el) =>el.innerText));      //Находит кол-во голосов
    const date = await page.$$eval('div.prem_list > div > div.premier_item > meta:nth-child(1)', postLinks => postLinks.map((el) =>el.content));        //Находит даты премьеры
    const company = await page.$$eval('div.prem_list > div > div.premier_item > div.prem_day > s', postLinks => postLinks.map((el) =>el.innerText));    //Находит компании, выпускающии фильмы
    const genres =[];
    let j=0;
    for (let url of film_link) {    //Переходит по всем ссылкам на фильмы и там находит их жанры
        while(true){
            try{
                await page.goto(url);
                g = await page.$eval('div.styles_root__B1q5W.styles_root__axj8R > div.styles_root__UtArQ > div > div.styles_column__r2MWX.styles_md_17__FaWtp.styles_lg_21__YjFTk.styles_column__5dEFP > div > div > div:nth-child(2) > div.styles_column__r2MWX.styles_md_11__UdIH_.styles_lg_15__Ai53P > div > div:nth-child(3) > div.styles_value__g6yP4', (el) =>el.innerText);
    ...
    fs.writeFileSync("./Middle_Task_1.json",JSON.stringify(films, null, '\t')); //Преобразует это все в json и создает или перезарисывает существующий
}
start()